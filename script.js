...
window.onload = function() {
  ...
  // Start confetti on load and set timer to stop after 10 seconds
  poof();
  setTimeout(() => {
    clearTimeout(timer); // Stop adding new confetti
    timer = undefined;
  }, 10000);

  // Button functionality to display the present and trigger confetti
  ...
  presentButton.addEventListener('click', function() {
    presentMessage.style.display = 'block'; // Display the message
    presentButton.style.display = 'none'; // Hide the button
    poof(); // Trigger confetti again
  });
};
